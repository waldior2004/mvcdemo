...
namespace com.msc.infraestructure.dal
{
    public partial class Repository
    {
        ...
        {
            List<GrupoTabla> lst = null;
            try
            {
                using (var context = new CompanyContext())
                {
                    lst = (from p in context.GrupoTablas
                           where p.AudActivo == 1
                           select p).ToList();
                }
                return lst;
        ...
        public GrupoTabla ObtGrupoTabla(int Id)
        {
            ...
            try
            {
                using (var context = new CompanyContext())
                {
                    obj = (from p in context.GrupoTablas
        ...
        public Respuesta EditGrupoTabla(GrupoTabla obj)
        {
            ...
            try
            {
                using (var context = new CompanyContext())
                {
                    if (obj.Id == 0)
                    {
                        var codeex = (from p in context.GrupoTablas
                                      where (p.Codigo == obj.Codigo || p.Descripcion.ToLower() == obj.Descripcion.ToLower()) && p.AudActivo == 1 && p.Id != obj.Id
                                      select p).FirstOrDefault();
                        ...
                        {
                            obj.AudActivo = 1;
                            context.GrupoTablas.Add(obj);
                            objResp = MessagesApp.BackAppMessage(MessageCode.InsertOK);
                        }
                    }
                    else
                    {
                        var exists = (from p in context.GrupoTablas
        ...
        public Respuesta ElimGrupoTabla(int Id)
        {
            ...
            try
            {
                using (var context = new CompanyContext())
                {
                    ...
                    {
                        var tablas = (from p in context.Tablas
                                         where p.IdGrupoTabla == Id
                                         select p);
