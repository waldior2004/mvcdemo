...
namespace com.msc.frontend.mvc.Controllers
{
    public class PerfilController : Controller
    {
        ...
        public ActionResult Index()
        {
            try
            {
                var result = (HttpContext.Application["proxySeguridad"] as ISeguridad).ObtPerfil();
                foreach (var item in result)
                {
                    lst.Add(item.SetPerfil());
                }
                return View(lst);
            }
            catch (Exception ex)
        ...
        public ActionResult View(int id)
        {
            try
            {
                var result = (HttpContext.Application["proxySeguridad"] as ISeguridad).ObtPerfil(id);
                return View(result.SetPerfil());
            }
            catch (Exception ex)
            {
                LogError.PostErrorMessage(ex, null);
        ...
        public ActionResult New()
        {
            try
            {
                ...
                this.loadSelectPaginas(lst, 0);
                return View("Edit", new Perfil { Id = 0, Descripcion = "" });
            }
            catch (Exception ex)
            {
                LogError.PostErrorMessage(ex, null);
        ...
        public JsonResult New(Perfil obj)
        {
            try
            {
                ...
                {
                    result = (HttpContext.Application["proxySeguridad"] as ISeguridad).EditPerfil(obj.GetPerfilDTO()).SetRespuesta();
                }
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public JsonResult Edit(Perfil obj)
        {
            try
            {
                if (!ModelState.IsValid)
            ...
            try
            {
                if (id.IndexOf(",") >= 0)
                {
                    ...
                    foreach (var item in codes)
                    {
                        if (item != "")
                        {
                            result = (HttpContext.Application["proxySeguridad"] as ISeguridad).ElimPerfil(Convert.ToInt32(item)).SetRespuesta();
                            if (result.Id == 0)
                            {
                                OK++;
                    ...
                    result = (HttpContext.Application["proxySeguridad"] as ISeguridad).ElimPerfil(Convert.ToInt32(id)).SetRespuesta();

                result.Metodo = "/Perfil/Index";
                return Json(result);
            }
            catch (Exception ex)
            {
