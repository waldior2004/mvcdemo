...
{
    public class TarifarioDocController : Controller
    {
        List<TarifarioDoc> lst = new List<TarifarioDoc>();
        ...
        public ActionResult View(int id)
        {
            try
            {
                ...
                var res = (HttpContext.Application["proxySistema"] as ISistema).ObtTarifario(id);
                var objR = res.SetTarifario();
                return View(objR.TarifarioDocs);
            }
            catch (Exception ex)
            {
                LogError.PostErrorMessage(ex, null);
        ...
        public JsonResult ViewDocs(string id)
        {
            ...
            var res = (HttpContext.Application["proxySistema"] as ISistema).ObtTarifario(Convert.ToInt32(id));
        ...
        public JsonResult Delete(string id)
        {
            try
            {
                result = (HttpContext.Application["proxySistema"] as ISistema).ElimTarifarioDoc(Convert.ToInt32(id)).SetRespuesta();
