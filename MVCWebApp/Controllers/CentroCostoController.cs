...
namespace com.msc.frontend.mvc.Controllers
{
    public class CentroCostoController : Controller
    {
        ...
        public ActionResult Index()
        {
            try
            {
                var result = (HttpContext.Application["proxySistema"] as ISistema).ObtCentroCosto();
                foreach (var item in result)
                {
                    lst.Add(item.SetCentroCosto());
                }
                return View(lst);
            }
            catch (Exception ex)
        ...
        public ActionResult View(int id)
        {
            try
            {
                var result = (HttpContext.Application["proxySistema"] as ISistema).ObtCentroCosto(id);
                return View(result.SetCentroCosto());
            }
            catch (Exception ex)
            {
                LogError.PostErrorMessage(ex, null);
        ...
        public ActionResult New()
        {
            try
            {
                return View("Edit", new CentroCosto { Id = 0, Descripcion = "" });
            }
            catch (Exception ex)
            {
                LogError.PostErrorMessage(ex, null);
        ...
        public JsonResult New(CentroCosto obj)
        {
            try
            {
                ...
                {
                    result = (HttpContext.Application["proxySistema"] as ISistema).EditCentroCosto(obj.GetCentroCostoDTO()).SetRespuesta();
                }

                result.Metodo = "/CentroCosto/Index";
                return Json(result);
            }
            catch (Exception ex)
            {
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public JsonResult Edit(CentroCosto obj)
        {
            try
            {
                if (!ModelState.IsValid)
            ...
            try
            {
                if (id.IndexOf(",") >= 0)
                {
                    ...
                    foreach (var item in codes)
                    {
                        if (item != "")
                        {
                            result = (HttpContext.Application["proxySistema"] as ISistema).ElimCentroCosto(Convert.ToInt32(item)).SetRespuesta();
                            if (result.Id == 0)
                            {
                                OK++;
                    ...
                    result = (HttpContext.Application["proxySistema"] as ISistema).ElimCentroCosto(Convert.ToInt32(id)).SetRespuesta();
