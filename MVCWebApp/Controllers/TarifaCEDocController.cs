...
{
    public class TarifaCEDocController : Controller
    {
        List<TarifaCEDoc> lst = new List<TarifaCEDoc>();
        ...
        public ActionResult View(int id)
        {
            try
            {
                ...
                var res = (HttpContext.Application["proxySistema"] as ISistema).ObtTarifaCE(id);
                var objR = res.SetTarifaCE();
                return View(objR.TarifaCEDocs);
            }
            catch (Exception ex)
            {
                LogError.PostErrorMessage(ex, null);
        ...
        public JsonResult ViewDocs(string id)
        {
            ...
            var res = (HttpContext.Application["proxySistema"] as ISistema).ObtTarifaCE(Convert.ToInt32(id));
        ...
        public JsonResult Delete(string id)
        {
            try
            {
                result = (HttpContext.Application["proxySistema"] as ISistema).ElimTarifaCEDoc(Convert.ToInt32(id)).SetRespuesta();
