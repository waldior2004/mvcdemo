...
namespace com.msc.frontend.mvc.Controllers
{
    public class RolController : Controller
    {
        ...
        public ActionResult Index()
        {
            try
            {
                var result = (HttpContext.Application["proxySeguridad"] as ISeguridad).ObtRol();
        ...
        public ActionResult View(int id)
        {
            try
            {
                var result = (HttpContext.Application["proxySeguridad"] as ISeguridad).ObtRol(id);
                return View(result.SetRol());
            }
            catch (Exception ex)
            {
                LogError.PostErrorMessage(ex, null);
        ...
        public ActionResult New()
        {
            try
            {
                return View("Edit", new Rol { Id = 0, Descripcion = "" });
            }
            catch (Exception ex)
            {
                LogError.PostErrorMessage(ex, null);
        ...
        public JsonResult New(Rol obj)
        {
            try
            {
                ...
                {
                    result = (HttpContext.Application["proxySeguridad"] as ISeguridad).EditRol(obj.GetRolDTO()).SetRespuesta();
                }

                result.Metodo = "/Rol/Index";
                return Json(result);
            }
            catch (Exception ex)
            {
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public JsonResult Edit(Rol obj)
        {
            try
            {
                if (!ModelState.IsValid)
            ...
            try
            {
                if (id.IndexOf(",") >= 0)
                {
                    ...
                    foreach (var item in codes)
                    {
                        if (item != "")
                        {
                            result = (HttpContext.Application["proxySeguridad"] as ISeguridad).ElimRol(Convert.ToInt32(item)).SetRespuesta();
                            if (result.Id == 0)
                            {
                                OK++;
                    ...
                    result = (HttpContext.Application["proxySeguridad"] as ISeguridad).ElimRol(Convert.ToInt32(id)).SetRespuesta();
