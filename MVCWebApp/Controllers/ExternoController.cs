...
namespace com.msc.frontend.mvc.Controllers
{
    ...
    {
        List<Externo> lst = new List<Externo>();
        ...
        public ActionResult Index()
        {
            try
            {
                var result = (HttpContext.Application["proxySistema"] as ISistema).ObtExterno();
                foreach (var item in result)
                {
                    lst.Add(item.SetExterno());
                }
                return View(lst);
            }
            catch (Exception ex)
            ...
            try
            {
                if (id.IndexOf(",") >= 0)
                {
                    ...
                    foreach (var item in codes)
                    {
                        if (item != "")
                        {
                            result = (HttpContext.Application["proxySistema"] as ISistema).ResetKeyExterno(Convert.ToInt32(item)).SetRespuesta();
                            if (result.Id == 0)
                            {
                                OK++;
                    ...
                    result = (HttpContext.Application["proxySistema"] as ISistema).ResetKeyExterno(Convert.ToInt32(id)).SetRespuesta();

                result.Metodo = "/Externo/Index";
                return Json(result);
            }
            catch (Exception ex)
            {
        ...
        public JsonResult New(Externo obj)
        {
            try
            {
                ...
                {
                    result = (HttpContext.Application["proxySistema"] as ISistema).EditExterno(obj.GetExternoDTO()).SetRespuesta();
                }
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public JsonResult Edit(Externo obj)
        {
            try
            {
                if (!ModelState.IsValid)
            ...
            try
            {
                if (id.IndexOf(",") >= 0)
                {
                    ...
                    foreach (var item in codes)
                    {
                        if (item != "")
                        {
                            result = (HttpContext.Application["proxySistema"] as ISistema).ElimExterno(Convert.ToInt32(item)).SetRespuesta();
                            if (result.Id == 0)
                            {
                                OK++;
                                Message += string.Format("OK({0})", item);
                    ...
                    result = (HttpContext.Application["proxySistema"] as ISistema).ElimExterno(Convert.ToInt32(id)).SetRespuesta();
